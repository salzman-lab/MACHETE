...
for reportfile in glob.glob(reportsDir+"*_naive_report.txt"):
    ...
    for line_raw in f1:
        if line_raw[0] =="@":
            fout.write(line_raw.strip()+"\t"+"_1 NoIndels:Indels \t_2 NoIndels:Indels\tBadFJ=1\n")
        ...
        if junc in JunctionIndelsDict:
            fout.write(line_raw.strip()+"\t"+JunctionIndelsDict[junc][0]+"\t"+JunctionIndelsDict[junc][1]+"\t"+ BadFJ +"\n")
        else:
            fout.write(line_raw)
