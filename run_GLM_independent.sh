...
if [[ "$Version" = UseIndel ]]
then
RSCRIPT=GLM_script_Mar30_UseIndel.r
fi
if [[ "$Version" = NoIndel ]]
then
...
j_id=`sbatch -J GLM.r ${RESOURCE_FLAG} --array=1-${NUM_FILES} --mem=55000 --nodes=4 --time=5:0:0 -o ${2}err_and_out/out_independentGLM_r.txt -e ${2}err_and_out/err_independentGLM_r.txt ${INSTALLDIR}run_GLM_independent_step2.sh ${1} ${2} ${UniqueIDList} ${RSCRIPT} | awk '{print $4}'`
