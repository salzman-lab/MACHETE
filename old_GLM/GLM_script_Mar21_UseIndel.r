...
junctionPredictions[, p_value:=(pnorm(log(1/max.iter - 1) - numReads*use_mu)/sqrt(numReads*use_var))]
...
read_phat = my_reads[,p_predicted]
logitPhat = log((1-read_phat)/read_phat) 

use_mu = mean(logitPhat) # this is actually the mean of the read level predictions
use_var=var(logitPhat)
## for large n, 
print ("using threshold of ** below** for p value approximated by ")
n.thresh.exact=2
print (n.thresh.exact)

junctionPredictions[ (numReads>n.thresh.exact) , p_value:=(pnorm(log(1/p_predicted -1) - numReads*use_mu)/sqrt(numReads*use_var))]
...
#hist(read_phat)
my.dist[[tempN]]=sample(logitPhat, n.sampled, replace=T)
}
...
print ("NEED BETTER CALCULATION OF MU DISTRIBUTION, CURRENTLY ERROR")
junctionPredictions [ (numReads == tempN ), p_value:= sum(1/(1+exp(sim.readps))>p_predicted)/length(1/(1+exp(sim.readps)))]
#junctionPredictions [ (numReads == tempN ), p_value:= sum(mu_dist>p_predicted)/length(mu_dist)]
...
class_input=args[2]
FJ_indels_class_input=args[3]
reg_indels_class_input=args[4]
srr= args[5]
output_dir=args[6]
}
...
fusion_class_input = paste("",srr,"_1__output_FJ.txt",sep="")
class_input = paste("",srr,"_1__output.txt",sep="")
output_dir=""
...
anomaly_fusion_reads = na.omit(myClassesFusion[(class %like% 'FJbad'), list(id, pos, qual, aScore, numN, readLen, junction, qualR2,aScoreR2, numNR2, readLenR2),]) # GILLIAN, pls comment here on what FJ bad is for the sake of documentation
...
linearWithAnomalyAndIndelFusionPredictions = predictNewClassP(linear_and_anomaly_and_indel_fusions)
}
