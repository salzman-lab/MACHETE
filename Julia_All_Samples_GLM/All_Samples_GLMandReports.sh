...
j1_id=`sbatch -J GLM_ewing.r ${RESOURCE_FLAG} --array=1-${NUM_FILES_Ewing} --mem=55000 --nodes=4 --time=5:0:0 -o ${Output_Ewing}err/out_GLM_r.txt -e ${Output_Ewing}err/err_GLM_r.txt ${INSTALLDIR}run_GLM.sh ${CircPipe_Ewing} ${FJDir_Ewing} ${INSTALLDIR} ${Output_Ewing}glmReports/ | awk '{print $4}'`
echo "Ewing GLM: ${j1_id}"


j2_id=`sbatch -J AppendNaiveRpt_ewing ${RESOURCE_FLAG} --array=1-${NUM_FILES_Ewing} --mem=55000 --nodes=4 --time=2:0:0 -o ${Output_Ewing}err/out_AppendNaive.txt -e ${Output_Ewing}err/err_AppendNaive.txt --depend=afterok:${j1_id} ${INSTALLDIR}AppendNaiveRept.sh ${FJDir_Ewing} ${CircPipe_Ewing}circReads/glmReports/ ${INSTALLDIR} ${Output_Ewing}glmReports/ ${Output_Ewing}AppendedReports/ | awk '{print $4}'`
...
j1_id=`sbatch -J GLM_bladder.r ${RESOURCE_FLAG} --array=1-${NUM_FILES_bladder} --mem=55000 --nodes=4 --time=5:0:0 -o ${Output_bladder}err/out_GLM_r.txt -e ${Output_bladder}err/err_GLM_r.txt ${INSTALLDIR}run_GLM.sh ${CircPipe_bladder} ${FJDir_bladder} ${INSTALLDIR} ${Output_bladder}glmReports/ | awk '{print $4}'`
echo "bladder GLM: ${j1_id}"


j2_id=`sbatch -J AppendNaiveRpt_bladder ${RESOURCE_FLAG} --array=1-${NUM_FILES_bladder} --mem=55000 --nodes=4 --time=2:0:0 -o ${Output_bladder}err/out_AppendNaive.txt -e ${Output_bladder}err/err_AppendNaive.txt --depend=afterok:${j1_id} ${INSTALLDIR}AppendNaiveRept.sh ${FJDir_bladder} ${CircPipe_bladder}circReads/glmReports/ ${INSTALLDIR} ${Output_bladder}glmReports/ ${Output_bladder}AppendedReports/ | awk '{print $4}'`
...
j1_id=`sbatch -J GLM_CML_test.r ${RESOURCE_FLAG} --array=1-${NUM_FILES_CML_test} --mem=55000 --nodes=4 --time=5:0:0 -o ${Output_CML_test}err/out_GLM_r.txt -e ${Output_CML_test}err/err_GLM_r.txt ${INSTALLDIR}run_GLM.sh ${CircPipe_CML_test} ${FJDir_CML_test} ${INSTALLDIR} ${Output_CML_test}glmReports/ | awk '{print $4}'`
echo "CML_test GLM: ${j1_id}"


j2_id=`sbatch -J AppendNaiveRpt_CML_test ${RESOURCE_FLAG} --array=1-${NUM_FILES_CML_test} --mem=55000 --nodes=4 --time=2:0:0 -o ${Output_CML_test}err/out_AppendNaive.txt -e ${Output_CML_test}err/err_AppendNaive.txt --depend=afterok:${j1_id} ${INSTALLDIR}AppendNaiveRept.sh ${FJDir_CML_test} ${CircPipe_CML_test}circReads/glmReports/ ${INSTALLDIR} ${Output_CML_test}glmReports/ ${Output_CML_test}AppendedReports/ | awk '{print $4}'`
...
j1_id=`sbatch -J GLM_prostate.r ${RESOURCE_FLAG} --array=1-${NUM_FILES_prostate} --mem=55000 --nodes=4 --time=5:0:0 -o ${Output_prostate}err/out_GLM_r.txt -e ${Output_prostate}err/err_GLM_r.txt ${INSTALLDIR}run_GLM.sh ${CircPipe_prostate} ${FJDir_prostate} ${INSTALLDIR} ${Output_prostate}glmReports/ | awk '{print $4}'`
echo "prostate GLM: ${j1_id}"


j2_id=`sbatch -J AppendNaiveRpt_prostate ${RESOURCE_FLAG} --array=1-${NUM_FILES_prostate} --mem=55000 --nodes=4 --time=2:0:0 -o ${Output_prostate}err/out_AppendNaive.txt -e ${Output_prostate}err/err_AppendNaive.txt --depend=afterok:${j1_id} ${INSTALLDIR}AppendNaiveRept.sh ${FJDir_prostate} ${CircPipe_prostate}circReads/glmReports/ ${INSTALLDIR} ${Output_prostate}glmReports/ ${Output_prostate}AppendedReports/ | awk '{print $4}'`
...
j1_id=`sbatch -J GLM_Engstrom.r ${RESOURCE_FLAG} --array=1-${NUM_FILES_Engstrom} --mem=55000 --nodes=4 --time=5:0:0 -o ${Output_Engstrom}err/out_GLM_r.txt -e ${Output_Engstrom}err/err_GLM_r.txt ${INSTALLDIR}run_GLM.sh ${CircPipe_Engstrom} ${FJDir_Engstrom} ${INSTALLDIR} ${Output_Engstrom}glmReports/ | awk '{print $4}'`
echo "Engstrom GLM: ${j1_id}"


j2_id=`sbatch -J AppendNaiveRpt_Engstrom ${RESOURCE_FLAG} --array=1-${NUM_FILES_Engstrom} --mem=55000 --nodes=4 --time=2:0:0 -o ${Output_Engstrom}err/out_AppendNaive.txt -e ${Output_Engstrom}err/err_AppendNaive.txt --depend=afterok:${j1_id} ${INSTALLDIR}AppendNaiveRept.sh ${FJDir_Engstrom} ${CircPipe_Engstrom}circReads/glmReports/ ${INSTALLDIR} ${Output_Engstrom}glmReports/ ${Output_Engstrom}AppendedReports/ | awk '{print $4}'`
...
j1_id=`sbatch -J GLM_SEQC.r ${RESOURCE_FLAG} --array=1-${NUM_FILES_SEQC} --mem=55000 --nodes=4 --time=5:0:0 -o ${Output_SEQC}err/out_GLM_r.txt -e ${Output_SEQC}err/err_GLM_r.txt ${INSTALLDIR}run_GLM.sh ${CircPipe_SEQC} ${FJDir_SEQC} ${INSTALLDIR} ${Output_SEQC}glmReports/ | awk '{print $4}'`
echo "SEQC GLM: ${j1_id}"


j2_id=`sbatch -J AppendNaiveRpt_SEQC ${RESOURCE_FLAG} --array=1-${NUM_FILES_SEQC} --mem=55000 --nodes=4 --time=2:0:0 -o ${Output_SEQC}err/out_AppendNaive.txt -e ${Output_SEQC}err/err_AppendNaive.txt --depend=afterok:${j1_id} ${INSTALLDIR}AppendNaiveRept.sh ${FJDir_SEQC} ${CircPipe_SEQC}circReads/glmReports/ ${INSTALLDIR} ${Output_SEQC}glmReports/ ${Output_SEQC}AppendedReports/ | awk '{print $4}'`
...
j1_id=`sbatch -J GLM_ov_salzman.r ${RESOURCE_FLAG} --array=1-${NUM_FILES_ov_salzman} --mem=55000 --nodes=4 --time=5:0:0 -o ${Output_ov_salzman}err/out_GLM_r.txt -e ${Output_ov_salzman}err/err_GLM_r.txt ${INSTALLDIR}run_GLM.sh ${CircPipe_ov_salzman} ${FJDir_ov_salzman} ${INSTALLDIR} ${Output_ov_salzman}glmReports/ | awk '{print $4}'`
echo "ov_salzman GLM: ${j1_id}"


j2_id=`sbatch -J AppendNaiveRpt_ov_salzman ${RESOURCE_FLAG} --array=1-${NUM_FILES_ov_salzman} --mem=55000 --nodes=4 --time=2:0:0 -o ${Output_ov_salzman}err/out_AppendNaive.txt -e ${Output_ov_salzman}err/err_AppendNaive.txt --depend=afterok:${j1_id} ${INSTALLDIR}AppendNaiveRept.sh ${FJDir_ov_salzman} ${CircPipe_ov_salzman}circReads/glmReports/ ${INSTALLDIR} ${Output_ov_salzman}glmReports/ ${Output_ov_salzman}AppendedReports/ | awk '{print $4}'`
...
j1_id=`sbatch -J GLM_CML_UConn.r ${RESOURCE_FLAG} --array=1-${NUM_FILES_CML_UConn} --mem=55000 --nodes=4 --time=5:0:0 -o ${Output_CML_UConn}err/out_GLM_r.txt -e ${Output_CML_UConn}err/err_GLM_r.txt ${INSTALLDIR}run_GLM.sh ${CircPipe_CML_UConn} ${FJDir_CML_UConn} ${INSTALLDIR} ${Output_CML_UConn}glmReports/ | awk '{print $4}'`
echo "CML_UConn GLM: ${j1_id}"


j2_id=`sbatch -J AppendNaiveRpt_CML_UConn ${RESOURCE_FLAG} --array=1-${NUM_FILES_CML_UConn} --mem=55000 --nodes=4 --time=2:0:0 -o ${Output_CML_UConn}err/out_AppendNaive.txt -e ${Output_CML_UConn}err/err_AppendNaive.txt --depend=afterok:${j1_id} ${INSTALLDIR}AppendNaiveRept.sh ${FJDir_CML_UConn} ${CircPipe_CML_UConn}circReads/glmReports/ ${INSTALLDIR} ${Output_CML_UConn}glmReports/ ${Output_CML_UConn}AppendedReports/ | awk '{print $4}'`
